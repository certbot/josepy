...
class FieldTest(unittest.TestCase):
    ...
    def test_no_omit_boolean(self):
        ...
        for default, omitempty, value in itertools.product(
            ...
            self.assertIs(Field("foo", default=default, omitempty=omitempty).omit(value), False)
...
class JSONObjectWithFieldsTest(unittest.TestCase):
    ...
    def test_encode(self):
        self.assertEqual(10, self.MockJSONObjectWithFields(x=5, y=0, z=0).encode("x"))
    ...
    def test_encode_serialization_error_passthrough(self):
        self.assertRaises(
            errors.SerializationError, self.MockJSONObjectWithFields(y=500, z=None).encode, "y"
        )
    ...
    def test_fields_from_json_fails_on_missing(self):
        self.assertRaises(
            errors.DeserializationError, self.MockJSONObjectWithFields.fields_from_json, {"y": 0}
        )
        self.assertRaises(
            errors.DeserializationError, self.MockJSONObjectWithFields.fields_from_json, {"Z": 0}
        )
        self.assertRaises(
    ...
    def test_fields_to_partial_json_encoder(self):
        self.assertEqual(
            self.MockJSONObjectWithFields(x=1, y=2, z=3).to_partial_json(), {"x": 2, "y": 2, "Z": 3}
        )

    def test_fields_from_json_decoder(self):
        self.assertEqual(
            {"x": 2, "y": 2, "z": 3},
            self.MockJSONObjectWithFields.fields_from_json({"x": 4, "y": 2, "Z": 3}),
        )
...
class DeEncodersTest(unittest.TestCase):
    ...
    def test_decode_b64jose_size(self):
        ...
        self.assertEqual(b"foo", decode_b64jose("Zm9v", size=3))
        self.assertRaises(errors.DeserializationError, decode_b64jose, "Zm9v", size=2)
        self.assertRaises(errors.DeserializationError, decode_b64jose, "Zm9v", size=4)

    def test_decode_b64jose_minimum_size(self):
        from josepy.json_util import decode_b64jose

        self.assertEqual(b"foo", decode_b64jose("Zm9v", size=3, minimum=True))
        self.assertEqual(b"foo", decode_b64jose("Zm9v", size=2, minimum=True))
        self.assertRaises(errors.DeserializationError, decode_b64jose, "Zm9v", size=4, minimum=True)

    def test_encode_hex16(self):
        from josepy.json_util import encode_hex16

        encoded = encode_hex16(b"foo")
        self.assertEqual("666f6f", encoded)
        self.assertIsInstance(encoded, str)
    ...
    def test_decode_hex16_minimum_size(self):
        ...
        self.assertEqual(b"foo", decode_hex16("666f6f", size=3, minimum=True))
        self.assertEqual(b"foo", decode_hex16("666f6f", size=2, minimum=True))
        self.assertRaises(errors.DeserializationError, decode_hex16, "666f6f", size=4, minimum=True)
...
class TypedJSONObjectWithFieldsTest(unittest.TestCase):
    ...
    def test_from_json_non_dict_fails(self):
        for value in [[], (), 5, "asd"]:  # all possible input types
            self.assertRaises(errors.DeserializationError, self.parent_cls.from_json, value)

    def test_from_json_dict_no_type_fails(self):
        self.assertRaises(errors.DeserializationError, self.parent_cls.from_json, {})

    def test_from_json_unknown_type_fails(self):
        self.assertRaises(errors.UnrecognizedTypeError, self.parent_cls.from_json, {"type": "bar"})

    def test_from_json_returns_obj(self):
        self.assertEqual({"foo": "bar"}, self.parent_cls.from_json({"type": "test", "foo": "bar"}))
